...
pub fn get_table_metadata(
  ...
  list.fold(tables, #(dict.new(), dict.new(), dict.new()), fn(acc, table) {
    add_table_metadata(acc, table, db)
  })
...
fn add_table_metadata(
  ...
  case query_table_info(db, table_name) {
    Ok(rows) -> {
      let columns = list.map(rows, column_from_table_info)
      let schemas = dict.insert(schemas, table_name, columns)
      let pks = add_table_pk(pks, table_name, rows)
...
fn column_from_table_info(row: #(String, String, Int, Int)) -> Column {
  let #(col_name, type_str, notnull, pk) = row
  let column_type = case query.parse_sqlite_type(type_str) {
    Ok(t) -> t
    Error(_) -> StringType
  }
  // SQLite quirk: INTEGER PRIMARY KEY columns have notnull=0 in PRAGMA output
  // because they're rowid aliases (which auto-assign on INSERT), but they're
  // always NOT NULL at read time.
...
fn add_table_pk(
  ...
  case list.find(rows, table_info_row_has_pk) {
    Ok(#(pk_name, _, _, _)) -> dict.insert(pks, table_name, pk_name)
    Error(_) -> pks
  }
}

fn table_info_row_has_pk(row: #(String, String, Int, Int)) -> Bool {
  row.3 > 0
}
