...
pub fn introspect_double_quoted_identifier_with_escaped_quotes_test() {
  ...
  let assert Ok(result) =
    sqlite.introspect_query(db, "test", "SELECT \"my\"\"col\", name FROM t")
...
pub fn introspect_double_quoted_keyword_identifier_in_where_test() {
  ...
  let assert Ok(result) =
    sqlite.introspect_query(db, "test", "SELECT id FROM t WHERE \"AND\" = ?")
  result
  |> string.inspect
...
pub fn introspect_select_distinct_single_column_test() {
  ...
  let assert Ok(result) =
    sqlite.introspect_query(db, "test", "SELECT DISTINCT status FROM t")
  result
  |> string.inspect
