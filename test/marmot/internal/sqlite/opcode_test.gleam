...
pub fn find_column_for_register_rowid_opcode_test() {
  ...
  let opcodes = [
    op(5, "OpenRead", 0, 1, 0),
    op(7, "Rowid", 0, 2, 0),
    // cursor=0, writes rowid to reg=2
    op(10, "ResultRow", 1, 2, 0),
  ]
  let cursor_table = cursor_map([#(0, "t")])
  let schemas = schema([col("id", IntType)])
...
pub fn find_column_for_register_seek_rowid_test() {
  let opcodes = [
    op(5, "OpenRead", 0, 1, 0),
    op(7, "SeekRowid", 0, 2, 0),
    // cursor=0, writes to reg=2
    op(10, "ResultRow", 1, 2, 0),
  ]
  let cursor_table = cursor_map([#(0, "t")])
  let schemas = schema([col("id", IntType)])
...
pub fn infer_parameter_eq_comparison_test() {
  ...
  let opcodes = [
    op(5, "OpenRead", 0, 1, 0),
    op(6, "Variable", 1, 1, 0),
    // param_num=1, writes to reg=1
    op(7, "Column", 0, 0, 2),
    // cursor=0, col_idx=0, writes to reg=2
    op(8, "Eq", 1, 0, 2),
    // compares reg 1 and reg 2, addr=8
    op(10, "ResultRow", 1, 2, 0),
  ]
...
pub fn debug_warning_suppressed_by_default_test() {
  ...
  let assert Nil = debug_warning("test warning")
}
